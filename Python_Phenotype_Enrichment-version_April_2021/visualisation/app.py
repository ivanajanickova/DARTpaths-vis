...
# Dash apps create a log of usage data as well as any print statements called from your app.
# These logs can be accessed via the command line. Need while development
import logging
# ===== START LOGGER =====
...
from sklearn.manifold import TSNE # dont need
...
# -------------------------------------------------------------------------------------------
# substitute for topics_txt
organism = ["celegans", "zebrafish", "mouse", "slimemould", "dmelanogaster"]
no_phenotype = [30,50,16,3,26]   # total number of phenotype enrich/pathway/organism
d = {'Organism':organism,'Total phenotype':no_phenotype}
df = pd.DataFrame(d)  # total number of phenotype enrich/pathway/organism substitute for journal_ser
journal = pd.Series(df['Total phenotype'].values, index=df['Organism'])
#-------------------------------------------
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [
                        dbc.Badge(
                            "Level of pathway(s):", color="info", className="mr-1"
                        ),
                        dbc.FormFloating(
                            [
                                dcc.Dropdown(
                        ...
                        dbc.Badge(
                            "Model organism(s):", color="info", className="mr-1"
                        ),
                        dbc.FormFloating(
                            [
                                dcc.Dropdown(
...
#                                       for i, v in journal_ser.items()   
                        ...
                        dbc.Badge("..... network:", color="info", className="mr-1"),
                        dbc.FormFloating(
                            [
                                dbc.Container(
                                    [
                        ...
                        dbc.FormFloating(
                            [
                                dcc.RadioItems(
                                    id="dim_red_algo",
                                    options=[
                                        {"label": "option 1", "value": "umap"},
                                        {"label": "option 2", "value": "tsne"},
                                    ],
                                    value="tsne",
                                    labelStyle={
                        ...
                        dbc.Container(
                            "Range of perplexity: .... (min: 10, max:100)",
                            id="tsne_para",
        ...
        dbc.Row(
            [
                dcc.Markdown(
                    """
            \* If we want to say something about the database/data we use, we can say here
            [Name of link for example](https://github.com/ivanajanickova/DARTpaths-vis)
            . The displayed nodes exclude phenotypes that .......
...
# -------------------------------------------------------------------------------------------
# call back
# callback functions: functions that are automatically called by Dash whenever an input component's property changes,
